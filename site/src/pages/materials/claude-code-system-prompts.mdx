...
description: 从 Claude Code 2.0 中提取的 43 个系统提示词详细解析 - 涵盖工具使用、安全协议、代理架构等核心机制
category: 工具深度解析
---
...
<div style={{
  ...
  <a href="#核心系统提示词" style={{ color: 'var(--md-ink)', textDecoration: 'none' }}>🎯 核心系统</a>
  <a href="#git-操作与版本控制" style={{ color: 'var(--md-ink)', textDecoration: 'none' }}>📝 Git 操作</a>
  <a href="#mcp-模型上下文协议-集成" style={{ color: 'var(--md-ink)', textDecoration: 'none' }}>🔌 MCP 集成</a>
  <a href="#文件操作工具" style={{ color: 'var(--md-ink)', textDecoration: 'none' }}>📁 文件操作</a>
  <a href="#搜索工具" style={{ color: 'var(--md-ink)', textDecoration: 'none' }}>🔍 搜索工具</a>
  <a href="#任务管理与-todo-列表" style={{ color: 'var(--md-ink)', textDecoration: 'none' }}>✅ 任务管理</a>
  <a href="#安全审查代理" style={{ color: 'var(--md-ink)', textDecoration: 'none' }}>🔐 安全审查</a>
  <a href="#统计摘要" style={{ color: 'var(--md-ink)', textDecoration: 'none' }}>📊 统计摘要</a>
</div>
...
在任何 `mcp-cli call` 之前,你**必须**调用 `mcp-cli info`。

这是一个**阻塞要求** - 就像你必须在 Edit 之前使用 Read 一样。
...
3. **文件和代码部分:** 列举检查、修改或创建的特定文件和代码部分。包含完整代码片段
...
假设此工具能够读取机器上的所有文件。如果用户提供文件路径,假设该路径有效。
...
- 提供更大的字符串和更多上下文以使其唯一
- 或使用 `replace_all` 更改每个 `old_string` 实例
...
<summary>markdown 摘要(仅在 should_summarize 为 true 时)</summary>
```
...
4. **逐字输出:** 逐字复制与对话上下文相关的输出。确保至少包含 3 个逐字输出片段
5. **不要提供建议** - 只总结事实
...
3. ⚠️ 初始任务:"添加一个处理用户身份验证的新功能" - 如果不确定认证方法(OAuth、JWT 等),首先使用 AskUserQuestion,然后在澄清方法后使用退出计划模式工具
...
* 建议: 使用 Flask 的 escape() 函数或启用自动转义的 Jinja2 模板
```
...
更多信息参见 [Claude Code action 仓库](https://github.com/anthropics/claude-code-action)。
...
<div style={{
  ...
  <div>
    <strong>指令提示词</strong><br/>
    5 个
  </div>
  <div>
    <strong>工具提示词</strong><br/>
    28 个
  </div>
  <div>
    <strong>其他提示词</strong><br/>
    9 个
  </div>
  <div>
...
- [Claude Code 2.0 深度架构分析](/materials/claude-code-architecture) - 完整的架构分析
...
查看 [Claude Code 2.0 深度架构分析](/materials/claude-code-architecture) 了解完整的逆向工程分析,包括 42 个 System Prompts 和 50+ 架构图。
