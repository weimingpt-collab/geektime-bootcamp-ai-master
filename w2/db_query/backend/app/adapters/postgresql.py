...
class PostgreSQLAdapter(DatabaseAdapter):
    ...
    async def stream_query(
        ...
        """Stream PostgreSQL query results using an asyncpg cursor.

        Uses `Connection.cursor()` which is a server-side cursor backed
        by the extended query protocol. Rows are fetched in batches of
        `batch_size` via `Cursor.fetch`, avoiding a full buffered
        `fetchall`. Stops as soon as `max_rows` total rows have been
        yielded.

        Args:
            sql: Validated SQL (caller is responsible for LIMIT injection)
            batch_size: Rows per fetch call (upper bound)
        ...
        # Effective per-fetch size must not exceed remaining budget.
        ...
        async with pool.acquire() as conn:
            ...
            while rows_yielded < max_rows:
                ...
                batch = await cursor.fetch(fetch_size)
                ...
                batch_rows: List[Dict[str, Any]] = [dict(record) for record in batch]
                yield batch_rows
                rows_yielded += len(batch_rows)

                # If we got fewer rows than requested, cursor is done.
