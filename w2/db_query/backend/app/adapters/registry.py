...
class DatabaseAdapterRegistry:
    ...
    def get_adapter(
        ...
        if existing is not None and existing.config.url == config.url:
            # Same URL - reuse cached instance (and its connection pool)
            return existing

        # URL changed or first creation - build a fresh adapter so the new
        # credentials/host are actually used. The old instance (if any) is
        # dropped; its pool will be cleaned up by close_adapter/close_all
        # or GC. This is critical for test_connection which reuses the
        # name "test" across different URLs.
        adapter_class = self._adapters[db_type]
        self._instances[cache_key] = adapter_class(config)
        logger.info(
            f"Created new {adapter_class.__name__} instance for {config.name} "
            f"(url changed or new)"
        )
