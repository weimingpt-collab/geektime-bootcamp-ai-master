...
import aiomysql
from aiomysql.cursors import SSDictCursor
...
class MySQLAdapter(DatabaseAdapter):
    ...
    def _parse_url(self, url: str) -> Dict[str, Any]:
        ...
        Username and password are percent-decoded so that credentials
        containing special characters (e.g. '@', ':', '/') can be passed
        safely when the URL is constructed client-side with encodeURIComponent.
        """
        parsed = urlparse(url)
        return {
    ...
    async def test_connection(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if not params.get('db'):
                # aiomysql requires a database; connect without one first
                # by falling back to None and letting MySQL reject if needed.
                params['db'] = None
    ...
    async def stream_query(
        ...
        """Stream MySQL query results using a server-side cursor.

        Uses aiomysql's `SSDictCursor` (server-side cursor) so the full
        result set is never buffered in client memory. Rows are fetched
        in batches of `batch_size` via `cursor.fetchmany`, stopping once
        `max_rows` total rows have been yielded.

        Requires `autocommit=True` (already set on the pool) because
        server-side cursors in MySQL must be declared within a
        transaction or autocommit context; SSDictCursor manages the
        DECLARE/FETCH sequence internally.

        Args:
            sql: Validated SQL (caller is responsible for LIMIT injection)
            batch_size: Rows per fetchmany call (upper bound)
            max_rows: Hard cap on total rows yielded

        Yields:
            List of row dicts (keyed by column name) per batch.
        """
        pool = await self.get_connection_pool()
        ...
        async with pool.acquire() as conn:
            # SSCursor requires autocommit=True (or an explicit tx).
            # The pool is created with autocommit=True above.
            async with conn.cursor(SSDictCursor) as cursor:
                await cursor.execute(sql)
                rows_yielded = 0

                while rows_yielded < max_rows:
                    remaining = max_rows - rows_yielded
                    fetch_size = min(effective_batch, remaining)
                    batch = await cursor.fetchmany(fetch_size)

                    if not batch:
                        break

                    # Normalize datetime values to ISO 8601 strings for
                    # consistent serialization in CSV/JSON output.
                    processed: List[Dict[str, Any]] = []
                    for row in batch:
                        proc_row: Dict[str, Any] = {}
                        for key, value in row.items():
                            if isinstance(value, datetime):
                                proc_row[key] = value.isoformat()
                            else:
                                proc_row[key] = value
                        processed.append(proc_row)

                    yield processed
                    rows_yielded += len(processed)

                    if len(processed) < fetch_size:
                        break
