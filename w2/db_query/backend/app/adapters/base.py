...
class DatabaseAdapter(ABC):
    ...
    async def stream_query(
        ...
        """Stream SQL query results in batches using a server-side cursor.

        Used for large result set exports. Unlike `execute_query`, this
        method must NOT buffer the full result set in memory; it should
        use a database-side cursor and yield rows in batches of
        `batch_size`. The first yielded batch's keys are treated as the
        column order by callers; subsequent batches share the same keys.

        Args:
            sql: SQL query string (already validated and transformed,
                including a LIMIT clause if the caller injected one)
            batch_size: Number of rows to fetch per yield. Implementations
                should respect this as an upper bound per chunk.
            max_rows: Hard cap on total rows yielded across all batches.
                Implementations must stop once this many rows have been
                emitted (inclusive).

        Yields:
            List[Dict[str, Any]]: A batch of rows, each row as a dict
                keyed by column name. Callers should not assume a
                non-empty first batch; queries that return zero rows
                may yield nothing.

        Raises:
            Exception: If cursor initialization or fetch fails.

        Example:
            async for batch in adapter.stream_query(sql, batch_size=1000, max_rows=50000):
                for row in batch:
                    process(row)
        """
        pass
